...
// GET /api/todos?userId=...&workspaceId=optional
...
router.post('/', async (req, res) => {
  try {
    const { content, userId, workspaceId } = req.body;
    if (!content || !userId) {
      return res.status(400).json({ error: 'content and userId are required' });
    }
    const data = { content, userId, completed: false };
    if (workspaceId) data.workspaceId = workspaceId;
    const todo = await prisma.quickTodo.create({ data });
...
router.patch('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { completed, content, workspaceId } = req.body;
    const updateData = {};
